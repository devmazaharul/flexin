...
export default function OrderSuccess() {
  ...
  useEffect(() => {
    if (!orderStore.orderID) return;
    setLoading(true)
    getorderDatawithOrderId(orderStore.orderID)
      .then((res) => {
  ...
  if (!orderInfo)
    return <p className="text-center py-20 text-gray-600">No order found <Link className='text-blue-400' href={'/products'}>Go to store</Link></p>;
        ...
        <div className="relative grid gap-6 p-8 md:grid-cols-[1.2fr_0.8fr] md:p-10">
          <div>
            ...
            <div className="mb-6 flex flex-wrap md:flex-nowrap w-full items-center gap-2">
              ...
              <Badge
                tone={
                  orderInfo.Payments.status !== 'PENDING' ? 'green' : 'amber'
                }
              >
